 ...
 */
static inline int32 fastfloor(float fp) {
    int32 i = (int32)fp;
    return (fp < i) ? (i - 1) : (i);
}
 ...
 */
static inline uint8 hash(int32 i) {
    return perm[(uint8)i];
}
...
static float grad(int32 hash, float x) {
    int32 h = hash & 0x0F;        // Convert low 4 bits of hash code
...
static float grad(int32 hash, float x, float y) {
    int32 h = hash & 0x3F;  // Convert low 3 bits of hash code
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise::fractal(uint32 octaves, float x) const {
    float output    = 0.f;
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise::fractal(uint32 octaves, float x, float y) const {
    float output = 0.f;
    float frequency = mFrequency;
    float amplitude = mAmplitude;
